    ...
    for _,row in df.iterrows():
        ...
        with col2:
            ...
            if st.button("Vote",key=row["roll_no"]):
                ...
                record_vote_block(user["roll_no"],row["candidate_name"])
...
def admin_dashboard():
    ...
    with tab1:
        ...
        if st.button("Add Candidate"):
            ...
            cursor.execute("SELECT * FROM candidates WHERE roll_no=?",(roll,))
...
def register():
    ...
    if st.button("Register"):
        ...
        success=add_user(roll,name,password,email,phone,path)
